...
get_ifconfig_route()
{
    ...
    if [ "/system/xbin/ip" != "ip" ]
    then
	echo "-- linux iproute2 --"
	/system/xbin/ip addr show     | grep -v valid_lft
	/system/xbin/ip route show
	/system/xbin/ip -6 route show | sed -e 's/expires [0-9]*sec //'
	return
    fi
	...
	Linux)
	   echo "-- linux / ifconfig --"
	   LANG=C /system/bin/ifconfig -a |egrep  "( addr:|encap:)"
	   LANG=C /bin/netstat -rn -4 -6
	   return
	   ;;
	FreeBSD|NetBSD|Darwin)
	   echo "-- FreeBSD/NetBSD/Darwin [MacOS X] --"
	   /system/bin/ifconfig -a | egrep "(flags=|inet)"
